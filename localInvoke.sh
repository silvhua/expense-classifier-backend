...
docker build --build-arg GOOGLE_APPLICATION_CREDENTIALS_PATH=application_default_credentials.json -t local . 
docker run -e GOOGLE_APPLICATION_CREDENTIALS_PATH=application_default_credentials.json -d -p 9000:8080 local:latest
